...
etcd_client = etcd.Client(host=app.config['ETCD_IP'], port=app.config['ETCD_PORT'])
...
def WebFormatTool(strj, ret, msg):
    ...
    return  flask.make_response(json.dumps(out, indent=4))
...
def delete_rc(url, rc, namespace, k8s_server):
    ...
    #get all pod of rc
    turl = 'http://{}:8080/api/v1/namespaces/{}/pods'
    pod_url = turl.format(k8s_server, namespace)
    ...
    for pod in strj['items']:
        ...
        #delete all pod
        turl = 'http://{}:8080/api/v1/namespaces/{}/pods/{}'
        purl = turl.format(k8s_server, namespace, pname)
...
def load_ports(k8s_server):

    namespace = DEFAULT_NAMESPACE
    pod_url = 'http://{}:8080/api/v1/namespaces/{}/pods'.format(k8s_server,namespace)
...
class PortManager(Resource):
    def get(self):
        port = all_ports.get()
        return WebFormatTool(str(port), True, '')

class IsPort(Resource):
    def get(self, port):
...
class HostRes2(Resource):
    def get(self):
        ...
        for sub_item in value._children:
            host = dict()
            #print os.path.basename(sub_item['key'])
            raw = json.loads(sub_item['value'])
            #print raw
            host['status'] = 'Running'
            host['cpu'] = raw['load5']
            host['mem'] = dict()
            host['mem']['usage'] = (raw['mem_total'] -
                                    raw['mem_available'])/oneMB
            host['mem']['total'] = raw['mem_total']/oneMB
            host['ip'] = raw['name']
            res.append(host)
...
class HostRes(Resource):
    def get(self):
        url = 'http://{}:8080/api/v1/nodes/'.format(app.config['CAAS_K8S_IP'])
...
class PodRes(Resource):
    def get(self):
        namespace = DEFAULT_NAMESPACE
        turl = 'http://{}:8080/api/v1/namespaces/{}/pods/' 
        url = turl.format(app.config['CAAS_K8S_IP'], namespace)
        r = requests.get(url)
        strj = r.json()
        #print json.dumps(strj,indent=4)
...
class PodInfo(Resource):
    ...
    pod_url='/api/v1/namespaces/{}/pods/'

    def get(self, namespace):

        server_url = ("http://{}:8080" + PodInfo.pod_url)
        server_url = server_url.format(app.config['CAAS_K8S_IP'], 
                namespace)
        para = request.args 
        print "paras %s, url %s " % (para, server_url)
        r = requests.get(server_url, params=para)
        r.headers['Access-Control-Allow-Origin'] = '*'
        if r.status_code != 200:
            response = flask.make_response(json.dumps({'kind': 'Status', 'code': r.status_code}))
            return response
        #return r.json()
        return WebFormatTool(r.json(), True, '')


class CaasManager(Resource):

    rc_url='/api/v1/namespaces/{}/replicationcontrollers/{}'

    # detail of this rc 
    def get(self, namespace, rc_name):

        server_url = ("http://{}:8080" + CaasManager.rc_url)
        server_url = server_url.format(app.config['CAAS_K8S_IP'], 
                namespace, rc_name)
        para = request.args 
    ...
    #delete rc
    def delete(self, namespace, rc_name):

        server_url = ("http://{}:8080" + CaasManager.rc_url)
        server_url = server_url.format(app.config['CAAS_K8S_IP'], 
                namespace, rc_name)

        r = delete_rc(server_url, rc_name, namespace, app.config['CAAS_K8S_IP'])
...
class CaasList(Resource):

    rc_url='/api/v1/namespaces/{}/replicationcontrollers'

    #list rc of this namespace
    def get(self, namespace):

        server_url = ("http://{}:8080" + CaasList.rc_url)
        server_url = server_url.format(app.config['CAAS_K8S_IP'], namespace)
    ...
    def post(self, namespace):
        ...
        print "post input url%s" % server_url 
        r = requests.post(server_url, data=para)
        ...
        if r.status_code != 200:
            ...
            return response
        #return r.json()
        response = WebFormatTool(r.json(), True, '')
        return response

class CaasInfo(Resource):

    url='/api/v1/namespaces'

    def get(self):
        names_list = []
        server_url = ("http://{}:8080" + CaasInfo.url)
        server_url = server_url.format(app.config['CAAS_K8S_IP'])

	r = requests.get(server_url)

	#print "case info res:%s" % r.json()

        strj = r.json()
	for namespace in strj['items']:
		name = namespace['metadata']['name']
		names_list.append(name)

        response_json = {'kind': 'CaasList', 'items': names_list}
